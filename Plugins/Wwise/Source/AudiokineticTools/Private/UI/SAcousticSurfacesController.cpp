...
public:
	...
	void Construct(const FArguments& InArgs)
	{
		...
		[
			...
			.HAlign(HAlign_Left)
			.VAlign(VAlign_Center)
			.Padding(OverrideButtonPadding, 0)
			[
				SNew(SButton)
...
void SAcousticSurfacesController::Construct(const FArguments& InArgs, TArray<TWeakObjectPtr<UObject>> ObjectsBeingCustomized, IDetailLayoutBuilder* InLayoutBuilder)
{
	...
	FEditorDelegates::EditorModeIDExit.AddRaw(this, &SAcousticSurfacesController::OnEditorModeExited);
	FEditorDelegates::EditorModeIDEnter.AddRaw(this, &SAcousticSurfacesController::OnEditorModeEntered);
...
FAkPoly& SAcousticSurfacesController::GetAcousticSurfaceChecked(UAkSurfaceReflectorSetComponent* reflectorSet, int faceIndex)
{
...
void SAcousticSurfacesController::OnEditorModeEntered(const FEditorModeID& InEditorModeID)
{
	/* Ideally here we would just refresh the inspector using LayoutBuilder->ForceRefreshDetails().
	However, When we recieve this notification, the previous editor mode has not been cleared yet. So at this point, both are active. (See SLevelEditor::ToggleEditorMode).
	For example, when switching from geometry mode to edit mode: If we refreshed the inspector here, geometry mode would be 'active' in the construct function, and individual faces would still be selected, so we would end up applying the changes only to the 'ghost' selected faces.
	Instead, we need to track which editor mode is being selected, and re-evaluate which faces to use.
	*/
...
void SAcousticSurfacesController::OnEditorModeExited(const FEditorModeID& InEditorModeID)
{
	if (InEditorModeID == FEditorModeID(TEXT("EM_Geometry")))
	{
		ApplyToAllFaces = true;
